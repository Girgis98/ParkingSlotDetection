...
class MarkingPointDetector(nn.Module):
    ...
    def __init__(self):
        """
        initialization of Marking Point Detector class
        """
    ...
    def forward(self, x):
        """
        :param x: input value for CNN

        :return: out: final output after passing through CNN
        """
        out = self.features(x)
...
def image_predict_marking_points(input_image, model):
    ...
    model = model.eval()
    predict_awal = predict(input_image.reshape((1, 3, 512, 512)).to(device),model,device)
